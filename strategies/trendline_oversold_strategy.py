...
策略逻辑:
1. 两次筑底形成后，等待一周
2. 超跌时全仓买入（RSI<30 或 价格跌破支撑线）
3. 超涨且价量不匹配时卖出10%（RSI>70 + 量比缩小）
4. 每次超跌补满全仓
5. 止损线为买入均价的-10%
"""
...
class TrendlineOversoldStrategy:
    ...
    def __init__(self, params: Optional[Dict] = None):
        ...
        # 策略参数
        self.min_bottom_interval = self.params.get("min_bottom_interval", 30)
        self.wait_after_bottom = self.params.get("wait_after_bottom", 5)  # 筑底后等待天数（一周≈5交易日）
        ...
        self.support_break_threshold = self.params.get("support_break_threshold", 0.02)  # 跌破支撑2%算超跌

        # 价量配合
        self.volume_shrink_threshold = self.params.get("volume_shrink_threshold", 0.8)  # 量比缩小到80%以下
    ...
    def _check_bottom_formation(self, bottom_result) -> bool:
        """检查是否形成有效筑底"""
        ...
        # 间隔需要超过指定天数，且底部有效
        return (interval >= self.min_bottom_interval and
                bottom_result.get("is_valid", 0) == 1)
    ...
    def analyze(self, data: pd.DataFrame) -> StrategySignal:
        ...
        if not self.state.bottom_confirmed:
            if self._check_bottom_formation(bottom_result):
        ...
        if self.state.bottom_confirmed and self.state.bottom_confirm_date:
            ...
            if days_since < self.wait_after_bottom:
                return StrategySignal(
                    ...
                    action=PositionAction.HOLD,
                    price=current_price,
                    reason=f"筑底确认后等待中 ({days_since}/{self.wait_after_bottom}天)",
                    details=details,
                )
            ...
            if is_oversold:
                if self.state.position == 0:
                    ...
                    return StrategySignal(
                        ...
                        price=current_price,
                        reason=f"超跌补仓至满仓: {', '.join(os_reasons)}",
                        details=details,
                    )
